...
public class FileMonitor {
    ...
    public FileMonitor(@Value("${logging.file.path}") String directory, @Value("${logging.file.name:spring.log}") String fileName) throws IOException {
            ...
            try {
                ...
                for (final WatchEvent<?> event : key.pollEvents()) {
                    ...
                    if (event.kind() == ENTRY_MODIFY && changed.equals(file)) {
                        logger.trace("monitor - ENTRY_MODIFY: " + changed);
                ...
                if (!isKeyStillValid) {
                    logger.trace("monitor - key is no longer valid: " + key);
                    listen.set(false);
                }
